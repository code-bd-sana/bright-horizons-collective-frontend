...
const toolbarSnippets: Record<string, string> = {
  Bold: '**bold text**',
  Italic: '*italic text*',
  H2: '\n## Heading 2\n',
  H3: '\n### Heading 3\n',
  ...
  Callout: '\n[callout]Key takeaway or safety note for parents[/callout]\n',
};

const toolbarActions = Object.keys(toolbarSnippets);
      ...
      >
        {(form) => {
          function handleToolbarClick(action: string) {
            const snippet = toolbarSnippets[action];
            if (!snippet) return;
            ...
            toast.message(`Inserted ${action} snippet.`);
          }
              ...
              <section className="mt-5 rounded-2xl border border-[#e7eceb] bg-white p-4 shadow-[0_4px_6px_rgba(0,0,0,0.06)] sm:p-5 2xl:p-6.25">
                ...
                <div className="mt-5 space-y-5">
                  <p className="max-w-179.5 font-manrope text-sm leading-5.25 text-[#607d8b]">
                    Use the rich text editor to compose the full content of this resource.
                    Formatting toolbar appears above the editor.
                  </p>
