...
function JourneyConnector({ vertical = false }: { vertical?: boolean }) {
  return (
    <span
      aria-hidden="true"
      className={`flex h-3.5 w-15.5 shrink-0 items-center gap-1 px-1 ${vertical ? 'my-3 rotate-90' : ''}`}
    >
...
function JourneyStep({ number, compact = false }: { number: number; compact?: boolean }) {
  const step = (
    <article className="relative h-47.5 w-50 shrink-0">
      <Image
      ...
      />
      <div className="absolute left-13.25 top-17.5 flex w-23.5 flex-col items-center gap-2 text-center">
        ...
        <h3
          className={`w-full font-nunito text-xs font-bold leading-3.75 text-[#3D3A4E] ${number === 1 || number >= 4 ? 'whitespace-nowrap' : ''}`}
        >
...
function TabletJourney() {
  return (
    <ol className="hidden w-full grid-cols-2 justify-items-center gap-x-8 gap-y-6 md:grid lg:grid-cols-3 2xl:hidden">
      {journeySteps.map((label, index) => (
...
export function AboutJourneySection() {
  return (
    <section
      ...
      className="bg-[#EBF5FF] px-5 py-16 sm:px-8 sm:py-20 md:px-12 md:py-24 lg:px-16 2xl:h-146 2xl:px-16 2xl:py-30"
    >
      <div className="mx-auto flex max-w-[1792px] flex-col items-center gap-10 sm:gap-12">
        <header className="flex w-full flex-col items-center gap-4 text-center">
          <span className="rounded-xl border border-[#D4D6D7] bg-[#FFFDF8] px-2 py-1.5 font-manrope text-sm leading-5.5 tracking-[-0.084px] text-[#515B60]">
            Member Journey
          </span>
          <h2
            id="about-journey-heading"
            className="font-nunito text-[32px] font-semibold leading-9.5 tracking-[-0.48px] text-[#263238] sm:text-4xl sm:leading-11 xl:text-[48px] xl:leading-14"
          >
